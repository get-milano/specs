...
This repository also ships a reference checker, `tools/reference_check.py`: a minimal implementation of the gate and the expression language derived from specs 01 through 03. CI runs every step-free vector through it, so a vector whose expectation contradicts the spec's semantics fails at merge instead of surfacing later in an engine's suite. The checker is a linter, not a third runtime: the prose remains normative, vectors with `steps` are covered only by the engines' drivers, and a disagreement between the checker and a vector signals a defect in one of them, resolved by a human editing whichever is wrong.
